...
Example: A 1 KB message with 1024-dim embedding uses ~9 KB. The 5 MB default limit holds ~500 such messages.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Conduct a retrieval test to check if BONCFlow can recover the intended content for the LLM.',
      Parse: 'Parse',
      dataset: 'Dataset',
      ...
      rerankTip: `Optional. If left empty, BONCFlow will use a combination of weighted keyword similarity and weighted vector cosine similarity; if a rerank model is selected, a weighted reranking score will replace the weighted vector cosine similarity. Please be aware that using a rerank model will significantly increase the system's response time. If you wish to use a rerank model, ensure you use a SaaS reranker; if you prefer a locally deployed rerank model, ensure you start BONCFlow with docker-compose-gpu.yml.`,
      topK: 'Top-K',
  ...
  <li>The auto-keyword feature is dependent on the LLM and consumes a significant number of tokens.</li>
    ...
    chat: {
      ...
      **Essential Rules:**
      ...
      betaError:
        'Please acquire a BONCFlow API key from the System Settings page first.',
      searching: 'Searching...',
      parsing: 'Parsing',
      uploading: 'Uploading',
...
Example: Virtual Hosted Style`,
      ...
      embeddingModelTip:
        'The default embedding model for each newly created knowledge base. If you cannot find an embedding model from the dropdown, check if you are using BONCFlow slim edition (which does not include embedding models) or check https://ragflow.io/docs/dev/supported_models to see if your model provider supports this model.',
      img2txtModel: 'VLM',
      img2txtModelTip:
...
[Insert text here]`,
    ...
    search: {
      ...
      mediaHint: 'Optional: upload an image or video for multimodal retrieval',
      selectedImage: 'Selected image',
      selectedVideo: 'Selected video',
      clearMedia: 'Clear media',
      mediaTypeError: 'Only image or video files are supported',
      mediaUploadError: 'Failed to process media file',
